...
`expo-video` does not support playing media directly from a result of calling the `require` function. First, the video has to be loaded using `expo-asset`:

```jsx
import { Asset } from 'expo-asset';
import { VideoSource } from 'expo-video';

const id = require('./assets/bigbuckbunny.mp4');
