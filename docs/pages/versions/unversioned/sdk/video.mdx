...
`expo-video` supports playing local media loaded using the `require` function. You can use the result as a source directly, or assign it to the `uri` parameter of a [`VideoSource`](#videosource) if you also want to configure other properties.

```tsx
import { VideoSource } from 'expo-video';

const asset = require('./assets/bigbuckbunny.mp4');
...
const player1 = useVideoPlayer(asset); // You can use the `asset` directly as a video source
