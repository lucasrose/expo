...
`expo-video` does not support playing media directly from a result of calling the `require` function. Use `expo-asset` to load the video before using it as the video source:

```tsx
import { Asset } from 'expo-asset';
import { VideoSource } from 'expo-video';

const asset = Asset.fromModule(require('./assets/bigbuckbunny.mp4'));

const bigBuckBunnySource: VideoSource = {
  // asset.localUri or asset.uri will be used depending on if the app was built in release or debug mode
...
// Use the `bigBuckBunnySource` as the video source
```
